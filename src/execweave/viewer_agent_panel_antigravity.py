...
function execweaveAntigravityChildRounds(messages,path){
  ...
  const isTask=message=>{
    ...
    return isObserved(message)&&!isInjected(message)&&String(message?.recipient||'')===path&&sender!==path&&(!sender||sender==='user'||sender===parent||sender.startsWith('antigravity:'))&&(role==='antigravity_addressed_task'||kind==='subagent_task'||kind==='send_message'||/task|assign/i.test(kind)||phase==='assignment');
  };
