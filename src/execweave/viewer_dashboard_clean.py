...
function execweaveDashboardGraph(data){
  ...
  function ownerFor(call){
    ...
    for(const edge of edges){
      ...
      if(source&&!hiddenTypes.has(String(source.type||''))&&source.type!=='tool')return source.source;
    }
    return null;
  }
