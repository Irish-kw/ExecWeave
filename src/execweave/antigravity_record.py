...
AntigravityRecordResult = ProviderRecordResult


def record_antigravity_to_viewer(
    command: list[str],
    *,
    watch_root: str | Path,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="execweave-antigravity-record",
        description="Record runtime evidence and Antigravity hook telemetry in one local run.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not command:
        parser.error(
            "an Antigravity CLI command is required, e.g. "
            "execweave-antigravity-record --open -- agy"
        )
    watch_root = (args.watch_root or Path.cwd()).expanduser().resolve()
    try:
        result = record_antigravity_to_viewer(
            command,
            watch_root=watch_root,
