...
def build_parser() -> argparse.ArgumentParser:
    ...
    event.add_argument("--runtime", choices=["ollama", "llamacpp"], required=True)
    ...
    probe = sub.add_parser("probe", help="Snapshot loaded models and optional aggregate runtime metrics.")
    probe.add_argument("--runtime", choices=["ollama", "llamacpp"], required=True)
    probe.add_argument("--endpoint", default=None)
    probe.add_argument("--sidecar", type=Path, default=None)
    probe.add_argument("--metrics", action="store_true", help="Also collect llama.cpp /metrics when enabled server-side.")
    probe.add_argument("--timeout", type=float, default=3.0)
    return parser


def _default_endpoint(runtime: str) -> str:
    return "http://localhost:11434" if runtime == "ollama" else "http://localhost:8080"
...
def _event_command(args: argparse.Namespace) -> int:
    ...
    if args.runtime == "ollama":
        ...
        records = llamacpp_response_to_events(
            payload,
            endpoint=endpoint,
            request_id=args.request_id,
        )
...
def _probe_command(args: argparse.Namespace) -> int:
    ...
    if args.runtime == "ollama":
        ...
        records.extend(llamacpp_models_to_events(payload, endpoint=endpoint))
        if args.metrics:
