...
_ACTION_TARGET_WITH_PAYLOADS = """      for(const target of [...group.targets].sort()){
      ...
      // Exact delegation nodes may own provider-observed prompt/description payloads.
      // When the task/subtask node is superseded by the orchestration action, preserve
      // those payloads on that action rather than leaving a dangling edge or dropping
      // user-visible full-fidelity evidence. Canonical raw edges are untouched.
      const delegationPayloadRelations=new Set(['HAS_SUBTASK_PROMPT','HAS_SUBTASK_DESCRIPTION']);
      for(const rawId of group.rawNodeIds){
        for(const rawEdge of outgoing.get(rawId)||[]){
          const payloadRelation=relation(rawEdge);
          if(!delegationPayloadRelations.has(payloadRelation))continue;
          if(!display.nodes.some(node=>node?.id===rawEdge.target))continue;
          addedEdges.push({
            id:`viewer:${id}--${payloadRelation}-->${rawEdge.target}:evidence:${encodeURIComponent(rawEdge.id||rawId)}`,
            source:id,target:rawEdge.target,relation:rawEdge.relation||payloadRelation,
