...
_REQUEST_RELATIONS = frozenset(
    {
        ...
        "OBSERVED_INFERENCE_REQUEST_CONFIG",
    ...
    *,
    ...
    try:
        ...
        if not isinstance(request, dict):
            request = {}
        if request_only and not any(key in request for key in ("messages", "prompt", "input")):
            return False
        ...
        if request_only:
            events = [event for event in events if event.get("relation") in _REQUEST_RELATIONS]
        elif request_recorded:
            events = [event for event in events if event.get("relation") not in _REQUEST_RELATIONS]
        for event in events:
            ...
            attrs.update(
                {
                    ...
                    "causal": False,
                    "inferred": False,
                    "capture_phase": "request" if request_only else "response",
                    "response_observed": not request_only,
                }
            )
        ...
        events.extend(
            raw_events[:1] if request_only else raw_events[1:] if request_recorded else raw_events
        )
        if request_only:
...
class ExecWeaveHTTPProxyHandler(BaseHTTPRequestHandler):
    ...
    def _relay(self) -> None:
        ...
        try:
            ...
            # Publish only dispatched request evidence, before waiting for model
            # headers/body. Custom recorder callbacks retain their one-call API.
            if self.server.recorder is record_exchange_fail_open:
                request_recorded = record_exchange_fail_open(
            ...
            if self.command != "HEAD":
                # read(n) can wait for n bytes or EOF, withholding flushed model
                # output until completion. read1 forwards available stream data.
