...
from .openai_compatible import (
    append_openai_compatible_records,
    response_to_events,
    sanitize_openai_compatible_endpoint,
)
from .openai_compatible_full_fidelity import (
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="execweave-openai-compatible",
        description=(
            "Capture explicit OpenAI-compatible direct API evidence plus complete content "
            "exposed by the selected integration point."
        ),
    )
    ...
    event = sub.add_parser(
        "event",
        help="Capture one supplied final API response; this is response-only evidence.",
    )
    event.add_argument("--endpoint", required=True)
    event.add_argument("--provider-name", default="openai-compatible")
    ...
    exchange.add_argument("--provider-name", default="openai-compatible")
...
def _append_full_fidelity_fail_open(sidecar: Path, records_factory) -> int:
    try:
        ...
        append_openai_compatible_records(sidecar, records)
        return len(records)
    except (OSError, RuntimeError, TimeoutError, TypeError, ValueError) as exc:
        print(f"ExecWeave OpenAI-compatible full-fidelity warning: {exc}", file=sys.stderr)
        return 0


def _event_command(args: argparse.Namespace) -> int:
    payload = _read_json_stdin()
    endpoint = sanitize_openai_compatible_endpoint(args.endpoint)
    ...
    output = append_openai_compatible_records(sidecar, records)
