...
function foldScope(node){const graph=window.__execweaveCore?.getGraph?.()||window.__execweaveStaticGraph||{};const session=String(graph.session_id||'');return session?`execweave:folds:v1:${foldHash(session)}:${foldHash(agentKey(node))}`:null}
...
function bindFold(fold,state,key,defaultOpen=false){
  ...
  fold.addEventListener('click',event=>{if(event.target.closest?.('summary')?.parentElement===fold)touched=true});
