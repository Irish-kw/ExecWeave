...
def _provider_session(payload: dict[str, Any]) -> dict[str, Any]:
    session_id = payload.get("session_id")
    if not isinstance(session_id, str) or not session_id:
        raise ValueError("Cursor session hook requires session_id")
    return _entity(
        "provider_session",
...
def _tool_call(payload: dict[str, Any], hook: str) -> dict[str, Any]:
    ...
    if isinstance(use_id, str) and use_id:
        return _entity(
            "tool_call",
            f"tool-call:cursor:{_scope(payload)}:{use_id}",
            name,
            provider="cursor",
            tool_name=name,
            tool_use_id=use_id,
    ...
    return _entity(
        ...
        f"tool-call-observation:cursor:{_scope(payload)}:{digest}",
        name,
        provider="cursor",
        tool_name=name,
...
def _subagent(payload: dict[str, Any], hook: str) -> dict[str, Any]:
    ...
    sub_type = (
        ...
        else "Cursor subagent"
    )
    if isinstance(sub_id, str) and sub_id:
        return _entity(
            "agent",
            f"agent:cursor:{_scope(payload)}:subagent:{sub_id}",
            sub_type,
            provider="cursor",
            subagent_id=sub_id,
            ...
            **subagent_topology(
                ...
                parent_scope_id=_scope(payload),
            ),
        )
    raw = json.dumps(
        [
    ...
    return _entity(
        ...
        f"subagent-observation:cursor:{_scope(payload)}:{digest}",
        sub_type,
        provider="cursor",
        identity_semantics="provider_hook_observation_without_subagent_id",
...
def _observe(
    ...
    **attrs: Any,
    ...
    ref = (
        store.put_text(value, content_kind=kind)
        if isinstance(value, str)
        else store.put_json(value, content_kind=kind)
    )
    events.append(
        content_observation_event(
            ...
            attributes={"cursor_hook_event_name": hook, **attrs},
        )
    )
    ...
    *,
    ...
    if hook == "beforeSubmitPrompt" and isinstance(payload.get("prompt"), str):
        _observe(
            ...
            kind="cursor.prompt_submission_candidate",
            timestamp=ts,
            source=_agent(),
            relation="OBSERVED_PROMPT_SUBMISSION_CANDIDATE",
            field="prompt",
            hook=hook,
            backend_submission_asserted=False,
            prompt_can_be_blocked_by_hook=True,
        )

    if hook == "sessionEnd" and isinstance(payload.get("error_message"), str):
        _observe(
            events,
            store,
            value=payload["error_message"],
            kind="cursor.session_end_error",
            timestamp=ts,
            source=_provider_session(payload),
            relation="OBSERVED_SESSION_END_ERROR",
            field="error_message",
            hook=hook,
            provider_reported_session_error=True,
        )
    ...
    if hook in {"preToolUse", "postToolUse", "postToolUseFailure"}:
        ...
        if "tool_input" in payload:
            _observe(
                ...
                kind="cursor.tool_input",
                timestamp=ts,
                source=call,
                relation="OBSERVED_TOOL_INPUT",
                field="tool_input",
                hook=hook,
            )
        if isinstance(payload.get("agent_message"), str):
            _observe(
                events,
                store,
                value=payload["agent_message"],
                kind="cursor.agent_message",
                timestamp=ts,
                source=call,
                relation="OBSERVED_AGENT_MESSAGE",
                field="agent_message",
                hook=hook,
            )
        ...
        if (
            ...
            _observe(
                ...
                store,
                value=payload["error_message"],
                kind="cursor.tool_failure",
                timestamp=ts,
                source=call,
                relation="RECEIVED_TOOL_ERROR",
                field="error_message",
                hook=hook,
            )
    ...
    if hook in {"beforeShellExecution", "afterShellExecution"}:
        if isinstance(payload.get("command"), str):
            relation = (
                "OBSERVED_SHELL_COMMAND_BEFORE_EXECUTION"
                if hook.startswith("before")
                else "OBSERVED_SHELL_COMMAND_AFTER_EXECUTION"
            )
            _observe(
                events,
                store,
                value=payload["command"],
                kind="cursor.shell_command",
                timestamp=ts,
                source=_agent(),
                relation=relation,
                field="command",
        ...
        if hook == "afterShellExecution" and isinstance(payload.get("output"), str):
            _observe(
                ...
                kind="cursor.shell_output",
                timestamp=ts,
                source=_agent(),
                relation="RECEIVED_SHELL_OUTPUT",
                field="output",
                hook=hook,
            )
    ...
    if hook in {"beforeMCPExecution", "afterMCPExecution"}:
        if isinstance(payload.get("command"), str):
            _observe(
                ...
                store,
                value=payload["command"],
                kind="cursor.mcp_server_command",
                timestamp=ts,
                source=_agent(),
                relation="OBSERVED_MCP_SERVER_COMMAND",
                field="command",
                hook=hook,
        ...
        if "tool_input" in payload:
            _observe(
                ...
                store,
                value=payload["tool_input"],
                kind="cursor.mcp_tool_input",
                timestamp=ts,
                source=_agent(),
                relation="OBSERVED_MCP_TOOL_INPUT",
                field="tool_input",
                hook=hook,
            )
        if hook == "afterMCPExecution" and "result_json" in payload:
            _observe(
                events,
                store,
                value=payload["result_json"],
                kind="cursor.mcp_tool_result",
                timestamp=ts,
                source=_agent(),
                relation="RECEIVED_MCP_TOOL_RESULT",
                field="result_json",
                hook=hook,
            )

    if (
        hook in {"beforeReadFile", "beforeTabFileRead"}
        ...
        _observe(
            ...
            kind="cursor.file_content_before_read",
            timestamp=ts,
            source=_file(payload),
            relation="OBSERVED_FILE_CONTENT_BEFORE_READ",
            field="content",
            hook=hook,
            read_completion_asserted=False,
        )
    if (
        hook in {"afterFileEdit", "afterTabFileEdit"}
        and isinstance(payload.get("edits"), list)
    ):
        _observe(
            events,
            store,
            value=payload["edits"],
            kind="cursor.file_edits",
            timestamp=ts,
            source=_file(payload),
            relation="OBSERVED_FILE_EDITS",
            field="edits",
            hook=hook,
    ...
    if (
        hook in {"afterAgentResponse", "afterAgentThought"}
        ...
        _observe(
            ...
            value=payload["text"],
            kind=(
                "cursor.agent_thought"
                if thought
                else "cursor.assistant_response"
            ),
            timestamp=ts,
            source=_agent(),
            relation=(
                ...
                if thought
                else "PRODUCED_ASSISTANT_RESPONSE"
            ),
            field="text",
            hook=hook,
            **({"provider_labels_as_thinking_text": True} if thought else {}),
        )

    if hook in {"subagentStart", "subagentStop"}:
        sub = _subagent(payload, hook)
        for field, relation in (
            ("task", "OBSERVED_SUBAGENT_TASK"),
            ("description", "OBSERVED_SUBAGENT_DESCRIPTION"),
            ("summary", "RECEIVED_SUBAGENT_SUMMARY"),
        ):
            if (
                isinstance(payload.get(field), str)
                and (field == "task" or hook == "subagentStop")
            ):
                _observe(
                    events,
                    store,
                    value=payload[field],
                    kind=f"cursor.subagent_{field}",
                    timestamp=ts,
                    source=sub,
                    relation=relation,
                    field=field,
