...
.controls{position:absolute;top:12px;left:12px;z-index:5;display:flex;gap:6px}.controls button,.timeline button{border:1px solid var(--border);background:var(--panel);color:var(--text);border-radius:7px;padding:6px 9px;cursor:pointer}.controls button:hover,.timeline button:hover{border-color:var(--selected)}
...
const timeline=document.getElementById('timeline'),sequenceFilter=document.getElementById('sequence-filter'),sequenceLabel=document.getElementById('sequence-label'),playButton=document.getElementById('timeline-play');
const sequenceOf=(edge,key)=>Number.isInteger(edge[key])?edge[key]:null;
const maxSequence=Math.max(0,...allEdges.map(edge=>sequenceOf(edge,'last_sequence')??sequenceOf(edge,'first_sequence')??0));
let selectedSequence=maxSequence,playTimer=null,visibleNodes=[],visibleEdges=[],nodeById=new Map(),positions=new Map(),nodeElements=new Map(),edgeElements=[];
...
function applyGraphFilters(){
  ...
  let edges=allEdges.filter(e=>ids.has(e.source)&&ids.has(e.target)&&(!relation||e.relation===relation)&&(!causal||e.causal===true)&&edgeExistsAt(e,selectedSequence));
...
function updateStats(){const seq=maxSequence>0?` · seq ${selectedSequence}/${maxSequence}`:'';stats.textContent=`${visibleNodes.length}/${allNodes.length} nodes · ${visibleEdges.length}/${allEdges.length} edges · ${graph.event_count??0} events${seq}`}
...
search.addEventListener('input',applySearch);[typeFilter,relationFilter,causalFilter].forEach(el=>el.addEventListener('change',applyGraphFilters));sequenceFilter.addEventListener('input',()=>{stopPlayback();setSequence(sequenceFilter.value)});playButton.addEventListener('click',togglePlayback);document.getElementById('fit').addEventListener('click',fit);document.getElementById('reset').addEventListener('click',()=>{computeLayout();renderNodes();renderEdges();applySearch();fit()});window.addEventListener('resize',fit);svg.addEventListener('click',()=>document.querySelectorAll('.node.selected').forEach(el=>el.classList.remove('selected')));
applyGraphFilters();
})();
</script>
</body>
