...
from .viewer_dashboard_projection import simplify_dashboard_graph
...
def project_viewer_graph(graph: dict[str, Any]) -> dict[str, Any]:
    return simplify_dashboard_graph(_base_project_viewer_graph(graph))


def _render_enriched_graph_html(graph: dict[str, Any], *, conversation_entries: list[dict[str, Any]] | None = None) -> str:
    projected = project_viewer_graph(graph)
    html = inject_standalone_execution_inspector(_base_render_graph_html(projected))
...
def write_graph_html(graph: dict[str, Any], path: str | Path, *, open_browser: bool = False) -> Path:
    ...
    output.write_text(_render_enriched_graph_html(graph, conversation_entries=entries), encoding="utf-8")
    if open_browser:
