...
La version courante est **v0.7.6**.
...
**v0.7.6 — un panneau d’agent qui répond à ce que l’agent a dit, et des réponses en flux reconstruites en un seul enregistrement.** Sélectionner un agent ouvre désormais sur ses propres turns, lus comme qui a dit quoi, en conservant les conversations multi-agent provider-neutral et agent-local que chaque agent possédait déjà. Les turns consécutifs qu’un provider n’a pas exposés se replient en une ligne qui nomme toujours les agents destinataires, et le préambule de plusieurs kilo-octets qu’un provider ajoute devant chaque subagent est plié plutôt que placé en tête du panneau ; l’evidence du node et la trace restent à une divulgation de distance. Les libellés du graphe résolvent le path déclaré de chaque agent, si bien que des frères créés dans la même milliseconde ne s’affichent plus comme le même fragment d’un id ordonné par le temps. En dessous, une réponse en flux est réassemblée dans le même enregistrement canonique que produirait sa jumelle non diffusée : texte, reasoning et tool calls répartis sur plusieurs frames sont rejoints avant toute matérialisation, et un flux interrompu est consigné comme interrompu.
