...
**v0.7.6 — その agent が何を言ったかに答える agent panel と、ひとつのレコードに再構成される streamed response。** agent を選択すると、その agent 自身の turn が「誰が何を言ったか」として最初に表示され、各 agent がすでに保持していた provider-neutral かつ agent-local な multi-agent conversation を引き継ぎます。provider が公開しなかった連続する turn は 1 行にまとめられますが、宛先の agent は列挙されたままです。provider がすべての subagent に前置きする数キロバイトの内容は 折りたたまれ、パネルの先頭を占めません。node evidence と trace も開示ひとつ分の距離にあります。Graph label は各 agent が 宣言した path を解決するため、同一ミリ秒で生成された兄弟が同じ time-ordered id の断片として描画されることはありません。内部では streamed response が非ストリーミング版と同じ canonical record に再構成され、frame をまたいで分割されたテキスト、reasoning、tool call は materialize の前に結合され、途中で終わったストリームは完了ではなく中断として記録されます。
