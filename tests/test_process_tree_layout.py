...
def test_live_html_embeds_dagre_directed_graph_layout() -> None:
    ...
    assert "execweaveRecomputePorts" in LIVE_HTML
    assert "dagre.layout" in LIVE_HTML
    assert "engine.layout(graph)" in LIVE_PROCESS_LAYOUT_SCRIPT
    assert "routePoints" in LIVE_PROCESS_LAYOUT_SCRIPT
...
def test_two_fixed_size_nodes_do_not_overlap_after_dagre_layout() -> None:
    ...
    script = (
        ...
        "global.edgeId=edge=>edge.id||`${edge.source}:${edge.target}`;\n"
        + layout_js
        + """
const topo={spec:new Map([['a',{x:0,y:0}],['b',{x:0,y:0}]]),sourcePort:new Map(),targetPort:new Map()};
...
const points=topo.routePoints.get('a\\0b')||[];
process.stdout.write(JSON.stringify({area:ix*iy,boxes,pointCount:points.length}));
"""
    )
    ...
    assert payload["pointCount"] >= 2, payload
...
def test_dagre_reorders_same_layer_nodes_to_reduce_edge_crossings() -> None:
    ...
    script = (
        ...
        "global.edgeById=new Map([['e1',{id:'e1',source:'p1',target:'c2'}],['e2',{id:'e2',source:'p2',target:'c1'}]]);\n"
        ...
        + layout_js
        + """
const topo={spec:new Map(ids.map(id=>[id,{x:0,y:0}])),sourcePort:new Map(),targetPort:new Map()};
execweaveApplyDirectedGraph(topo);
...
const port=topo.sourcePort.get('e1');
process.stdout.write(JSON.stringify({parentSign,childSign,area,port,y:{p1:y('p1'),p2:y('p2'),c1:y('c1'),c2:y('c2')}}));
"""
    )
    completed = subprocess.run(
        ["node", "-e", script, str(vendor)],
    ...
    assert payload["port"] and payload["port"]["total"] == 1, payload


def test_ports_follow_final_target_y_after_dagre_layout() -> None:
    start = LIVE_PROCESS_LAYOUT_SCRIPT.index("const EXECWEAVE_DAG_GAP=24;")
    end = LIVE_PROCESS_LAYOUT_SCRIPT.index("const execweaveBuildTopologyBase=execweaveBuildTopology;")
    layout_js = LIVE_PROCESS_LAYOUT_SCRIPT[start:end]
    vendor = SRC / "vendor" / "dagre.min.js"
    script = (
        "global.dagre=require(process.argv[1]);\n"
        "global.nodeById=new Map([['s',{id:'s'}],['hi',{id:'hi'}],['lo',{id:'lo'}]]);\n"
        "global.edgeById=new Map([\n"
        "  ['e-hi',{id:'e-hi',source:'s',target:'hi'}],\n"
        "  ['e-lo',{id:'e-lo',source:'s',target:'lo'}],\n"
        "]);\n"
...
const topo={
  ...
  sourcePort:new Map([['e-hi',{index:1,total:2}],['e-lo',{index:0,total:2}]]),
  targetPort:new Map(),
};
execweaveApplyDirectedGraph(topo);
const hiY=topo.spec.get('hi').y,loY=topo.spec.get('lo').y;
const hiPort=topo.sourcePort.get('e-hi').index,loPort=topo.sourcePort.get('e-lo').index;
const expectedHiLower=hiY<=loY?hiPort<loPort:hiPort>loPort;
process.stdout.write(JSON.stringify({hiY,loY,hiPort,loPort,expectedHiLower,points:topo.routePoints.size}));
"""
    )
    completed = subprocess.run(
        ["node", "-e", script, str(vendor)],
    ...
    payload = json.loads(completed.stdout)
    assert payload["expectedHiLower"], payload
    assert payload["points"] >= 1, payload