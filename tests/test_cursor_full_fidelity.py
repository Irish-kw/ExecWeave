...
def _event(events: list[dict], field: str) -> dict:
    return next(
        event
        for event in events
        if event["attributes"]["observed_field"] == field
    )


def test_prompt_submission_candidate_is_complete_and_not_inlined(
    tmp_path: Path,
) -> None:
    ...
    assert observed["relation"] == "OBSERVED_PROMPT_SUBMISSION_CANDIDATE"
    assert observed["attributes"]["backend_submission_asserted"] is False
    assert observed["attributes"]["prompt_can_be_blocked_by_hook"] is True
    assert prompt not in json.dumps(observed)


def test_session_end_error_is_preserved_outside_metadata(tmp_path: Path) -> None:
    events = cursor_hook_to_content_events(
        {
            **_base("sessionEnd"),
            "reason": "error",
            ...
            "final_status": "error",
            "error_message": "complete provider session error",
        },
        store=FullFidelityContentStore(tmp_path),
    )
    error = _event(events, "error_message")
    assert error["relation"] == "OBSERVED_SESSION_END_ERROR"
    assert error["source"]["id"] == "provider-session:cursor:session-1"
    assert _read(tmp_path, error) == "complete provider session error"
...
def test_shell_and_mcp_content_are_preserved(tmp_path: Path) -> None:
    ...
    mcp = cursor_hook_to_content_events(
        {
            **_base("afterMCPExecution"),
            "command": "node server.js",
            "tool_input": {"q": "x"},
            "result_json": {"answer": "full"},
...
def test_file_content_and_edits_keep_evidence_boundaries(tmp_path: Path) -> None:
    ...
    assert (
        edit_event["attributes"]["complete_post_edit_file_snapshot_asserted"]
        is False
    )


def test_assistant_thought_and_subagent_content_are_distinct(
    tmp_path: Path,
) -> None:
    store = FullFidelityContentStore(tmp_path)
    response = cursor_hook_to_content_events(
        {**_base("afterAgentResponse"), "text": "final answer"},
        store=store,
    )
    thought = cursor_hook_to_content_events(
        {**_base("afterAgentThought"), "text": "provider thought"},
        store=store,
    )
    assert _read(tmp_path, _event(response, "text")) == "final answer"
    assert (
        _event(thought, "text")["attributes"]["provider_labels_as_thinking_text"]
        is True
    )
    ...
    assert (
        _event(sub, "summary")["source"]["attributes"][
            "direct_start_stop_linkage_asserted"
        ]
        is False
    )
...
def test_transport_credentials_filtered_only_from_metadata(tmp_path: Path) -> None:
    payload = {
        **_base("preToolUse"),
        "tool_name": "Write",
        "tool_use_id": "w1",
        "tool_input": {"api_key": "keep-in-tool"},
    ...
    metadata = _read(
        ...
        next(
            event
            for event in events
            if event["relation"] == "OBSERVED_PROVIDER_METADATA"
        ),
    )
...
def test_cli_fail_open_keeps_summary_when_content_store_fails(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: (
            _ for _ in ()
        ).throw(RuntimeError("content failed")),
    )
